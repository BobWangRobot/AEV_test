...
def cal_and_plot(filename):
  ...
  for i in range(73, 100):
    thre = i/100
    x.append(thre)
    v5 = get_AEV_result(pdb_inp, cut_num=[thre, 5])
    AEV_method.append(flex.linear_correlation(x=v1, y=v5).coefficient())
    from_ca.append(flex.linear_correlation(x=v5, y=v2).coefficient())
    ksdssp.append(flex.linear_correlation(x=v5, y=v3).coefficient())
    cablam.append(flex.linear_correlation(x=v5, y=v4).coefficient())
